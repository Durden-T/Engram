...
import { Database, Play, Square, RefreshCw, CheckCircle2, AlertCircle, Loader2, Cpu, Hash, Zap } from "lucide-react";
import { embeddingService } from "@/services/rag";
import type { VectorConfig, EmbeddingConfig } from "@/services/api/types";
import type { StandaloneTriggerStatus } from "@/services/rag";
import { DEFAULT_EMBEDDING_CONFIG } from "@/services/api/types";
import { SettingsManager } from "@/services/settings/Persistence";
import { NumberField, SwitchField, SelectField } from "../APIPresets/components/FormField";
...
export const VectorizationPanel: React.FC = () => {
  ...
  // Standalone 配置状态
  const [triggerStatus, setTriggerStatus] = useState<StandaloneTriggerStatus | null>(null);
  ...
  // 更新 Standalone 配置
  const updateStandaloneConfig = (updates: Partial<NonNullable<EmbeddingConfig["standalone"]>>) => {
    const newStandaloneConfig = { ...config.standalone, ...updates };
    updateConfig({ standalone: newStandaloneConfig as NonNullable<EmbeddingConfig["standalone"]> });
  };

  // 加载 Standalone 触发状态
  const loadTriggerStatus = useCallback(async () => {
    if (config.trigger !== "standalone") {
      setTriggerStatus(null);
      return;
    }
    try {
      const { standaloneEmbeddingTrigger } = await import("@/services/rag/StandaloneEmbeddingTrigger");
      const status = await standaloneEmbeddingTrigger.getStatus();
      setTriggerStatus(status);
    } catch (e) {
      console.error("[VectorizationPanel] Failed to load trigger status:", e);
    }
  }, [config.trigger]);
  ...
  // 检查向量配置是否有效
  const isVectorConfigValid = vectorConfig && (vectorConfig.source === "transformers" || vectorConfig.model);
    ...
    <div className="py-4">
      ...
      {progress && (
        <div className="mb-6 p-4 bg-muted/30 rounded-lg">
          ...
          {progress.errors > 0 && <p className="text-xs text-destructive mt-2">{progress.errors} 个错误</p>}
        </div>
      )}
      ...
      <div className="space-y-4">
        ...
        <SelectField
          ...
          description="with_trim: Trim 时自动嵌入 | standalone: 独立阈值触发 | manual: 仅手动"
        />

        {/* Standalone 配置 (条件显示) */}
        {config.trigger === "standalone" && config.standalone && (
          <>
            <Divider className="my-4" />
            <div className="space-y-4">
              <h4 className="text-xs font-medium text-muted-foreground uppercase tracking-wider">独立触发设置</h4>

              {/* 触发类型选择 */}
              <div className="space-y-2">
                <label className="text-sm text-muted-foreground">触发类型</label>
                <div className="flex gap-6">
                  <label
                    className="flex items-center gap-2 cursor-pointer group"
                    onClick={() => updateStandaloneConfig({ triggerType: "count" })}
                  >
                    <span
                      className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors ${
                        config.standalone.triggerType === "count"
                    ...
                    <span
                      className={`text-sm transition-colors ${
                        config.standalone.triggerType === "count" ? "text-foreground" : "text-muted-foreground"
                      }`}
                    >
                      条目数
                    </span>
                  ...
                  <label
                    className="flex items-center gap-2 cursor-pointer group"
                    onClick={() => updateStandaloneConfig({ triggerType: "token" })}
                  >
                    <span
                      className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors ${
                        config.standalone.triggerType === "token"
                          ? "border-primary bg-primary"
                          : "border-border group-hover:border-muted-foreground"
                      }`}
                    >
                    ...
                    <span
                      className={`text-sm transition-colors ${
                        config.standalone.triggerType === "token" ? "text-foreground" : "text-muted-foreground"
                      }`}
                    >
                      Token 数
                    </span>
              ...
              {/* 阈值输入 */}
              <NumberField
                label={config.standalone.triggerType === "count" ? "条目数阈值" : "Token 阈值"}
                value={
                  config.standalone.triggerType === "count"
                    ? config.standalone.countLimit
                    : config.standalone.tokenLimit
                }
                onChange={(value) => {
                  const key = config.standalone!.triggerType === "count" ? "countLimit" : "tokenLimit";
                  updateStandaloneConfig({ [key]: value });
                }}
                min={config.standalone.triggerType === "count" ? 2 : 1024}
                max={config.standalone.triggerType === "count" ? 50 : 100000}
                description={
                  config.standalone.triggerType === "count"
                    ? "未嵌入事件数达到此值时自动触发"
                    : "未嵌入事件 Token 总数达到此值时自动触发"
                }
              />
              ...
              {triggerStatus && (
                <div className="p-3 bg-muted/30 rounded-lg space-y-2">
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">当前状态</p>
                  <div className="text-xs text-muted-foreground space-y-1">
                    <div className="flex justify-between">
                      <span>待嵌入事件:</span>
                      <span className="font-mono">{triggerStatus.pendingCount}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>当前{triggerStatus.triggerType === "token" ? " Token" : "条目数"}:</span>
                      <span className={`font-mono ${triggerStatus.triggered ? "text-amber-500 font-medium" : ""}`}>
                        {triggerStatus.currentValue} / {triggerStatus.threshold}
                      </span>
                    </div>
                    {triggerStatus.triggered && <p className="text-amber-500 text-xs mt-2">⚡ 已达到触发阈值</p>}
                  </div>
                </div>
              )}
            </div>
        ...
        {vectorConfig && (
          <div className="p-3 bg-muted/30 rounded-lg">
            ...
            <p className="text-sm">
              ...
              {vectorConfig.model && <span className="text-muted-foreground ml-2">/ {vectorConfig.model}</span>}
            </p>
          </div>
        )}
      </div>
