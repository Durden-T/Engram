...
import { batchProcessor, type BatchQueue, type HistoryAnalysis, type ImportMode } from '@/services/batch';
...
export const BatchProcessingPanel: React.FC = () => {
    ...
    const [endFloor, setEndFloor] = useState<number>(summarizerService.getStatus().currentFloor);
    ...
    const handleStart = useCallback(async () => {
        if (!analysis) return;
        await batchProcessor.start(analysis.startFloor, analysis.endFloor, (q) => setQueue({ ...q }));
    }, [analysis]);

    // 暂停/恢复
    const handlePauseResume = useCallback(() => {
        if (queue?.isPaused) batchProcessor.resume();
        else batchProcessor.pause();
    }, [queue]);

    // 停止
    const handleStop = useCallback(() => batchProcessor.stop(), []);
    ...
    const handleImport = useCallback(async () => {
        ...
        const result = await batchProcessor.importText(
            ...
            (q) => { if (q.tasks[0]) setImportProgress(q.tasks[0].progress); }
        );
    ...
    // 计算进度百分比
    const overallPercent = queue?.overallProgress
