...
export const MemoryStream: React.FC = () => {
  ...
  const handleBatchSave = async () => {
    ...
    try {
      const promises = Array.from(pendingChanges.entries()).map(([id, updates]) => store.updateEvent(id, updates));
  ...
  const selectAllState = useMemo(() => {
    if (filteredEvents.length === 0) return { checked: false, indeterminate: false };
    const allChecked = filteredEvents.every((e) => checkedIds.has(e.id));
    const someChecked = filteredEvents.some((e) => checkedIds.has(e.id));
    return {
      checked: allChecked,
      indeterminate: someChecked && !allChecked,
    };
  }, [filteredEvents, checkedIds]);
            ...
            >
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center h-48 text-muted-foreground gap-4">
                  ...
                  <p className="text-sm font-light">{searchQuery ? "没有找到匹配的事件" : "暂无记忆事件"}</p>
                </div>
              ) : (
