...
### 4.4 统一向量召回 (Unified Vector Retrieval) (V0.9.8)

不再区分事件与实体，所有记忆单元在同一向量空间中竞争。

```
Query -> Embedding
    ↓
Search Scope: [Event Vectors] U [Entity Vectors]
    ↓
Top-K Candidates (Mixed)
    ↓
Format:
...
> **理念**: 实体不仅仅是关键词，而是具有当前状态 (Description) 的长效记忆单元。
