...
Graph Builder: 自动从剧情摘要中提取实体和关系。

Graph Retriever: 基于图游走的上下文扩展算法。

Graph View: 基于 ReactFlow 的交互式图谱可视化。
...
创建 prompts/entity_extraction.md模板。
...
GraphRetriever Service:

位置：services/rag/GraphRetriever.ts

算法 (1-2 Hop Expansion):

Anchor: 使用 Embedding 找到 Top-K EventNodes。
...
维护 aliases 字段；构建时先检索相似实体名；必要时让 LLM 决定是 Merge 还是 Create。

图数据爆炸

限制每个 Event 关联的 Entity 数量 (Top-5)；定期清理孤立节点。

实时布局性能

Graph View 仅加载当前 Scope 的数据；支持仅显示最近 N 个事件的子图。
